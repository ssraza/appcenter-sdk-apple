...
class CrashesViewController : NSViewController, NSTableViewDataSource, NSTableViewDelegate, NSTextViewDelegate {
  ...
  func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
    if isHeader(row: row) {
      let categoryView = tableView.makeView(withIdentifier: convertToNSUserInterfaceItemIdentifier("crashName"), owner: nil) as! NSTextField
      ...
      case tableView.tableColumns[0]?:
        let nameView = tableView.makeView(withIdentifier: convertToNSUserInterfaceItemIdentifier("crashName"), owner: nil) as! NSTextField
        nameView.stringValue = (crashes[row] as! MSCrash).title
        return nameView
      case tableView.tableColumns[1]?:
        let crashButton = tableView.makeView(withIdentifier: convertToNSUserInterfaceItemIdentifier("crashButton"), owner: nil) as! NSButton
        crashButton.tag = row
